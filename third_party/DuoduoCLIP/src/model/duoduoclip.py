...
def _resolve_local_pretrained(base_name, pretrained_name):
    if os.path.exists(pretrained_name):
        return pretrained_name

    pretrained_cfg = custom_clip.get_pretrained_cfg(base_name, pretrained_name)
    if not pretrained_cfg:
        return pretrained_name

    hf_hub = pretrained_cfg.get('hf_hub', '').rstrip('/')
    if not hf_hub:
        return pretrained_name

    hub_dir_name = f"models--{hf_hub.replace('/', '--')}"

    for root in _candidate_cache_roots():
        model_dir = root / 'hub' / hub_dir_name
        snapshots_dir = model_dir / 'snapshots'
        if snapshots_dir.is_dir():
            snapshot_matches = sorted(snapshots_dir.glob('*/open_clip_pytorch_model.bin'))
            if snapshot_matches:
                return str(snapshot_matches[0])
        blobs_dir = model_dir / 'blobs'
        if blobs_dir.is_dir():
            blob_matches = sorted(path for path in blobs_dir.iterdir() if path.is_file())
            if blob_matches:
                return str(blob_matches[0])
...
def get_model(base_name='ViT-L-14', pretrained_name='laion2b_s32b_b82k'):
    ...
    resolved_pretrained = _resolve_local_pretrained(base_name, pretrained_name)
    if resolved_pretrained != pretrained_name:
        logger.info('Using local CLIP weights: %s', resolved_pretrained)

    model_pretrained, _, _ = custom_clip.create_model_and_transforms(base_name, pretrained=resolved_pretrained)
